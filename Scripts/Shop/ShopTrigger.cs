...
public partial class ShopTrigger : Area2D
{
    ...
    {
        if (_playerInside && @event.IsActionPressed("ui_accept"))
        {
            OpenShop();
    ...
    private void OnBodyEntered(Node2D body)
    {
        if (body is Player)
        {
            ...
            FloatingText.Spawn(GetParent() ?? this, GlobalPosition + new Vector2(0, -30), "[E] OPEN SHOP", new Color(0.95f, 0.8f, 0.2f), 14f);
        }
    }
    ...
    {
        if (_shopScene == null) return;

        if (_shopInstance == null || !GodotObject.IsInstanceValid(_shopInstance))
        {
            CanvasLayer uiLayer = GetTree().CurrentScene?.FindChild("UI", true, false) as CanvasLayer
            ...
            if (uiLayer != null)
            {
                uiLayer.AddChild(_shopInstance);
            }
            else
            {
